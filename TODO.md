...
package the runtime installs was checked against Alpine's secdb for v3.23 and
every one sits at or above its newest security fix. What follows did not have
an answer that could just be applied.

### The full image's ffmpeg cannot be fixed on Alpine 3.23

The two variants get their ffmpeg from different places, and only one of them
is current:

| variant                        | ffmpeg   | how                   |
| ------------------------------ | -------- | --------------------- |
| lean (`FFMPEG_VARIANT=source`) | 8.1.2    | compiled here, pinned |
| full (default, `apk`)          | 8.0.1-r1 | Alpine 3.23 community |

Upstream fixed three CVEs in the 8.0 branch after 8.0.1 — CVE-2025-67306 in
8.0.2, then CVE-2026-8461 and CVE-2026-30999 in 8.0.3. Alpine 3.23 is still on
8.0.1-r1, and its three patches are an RTSP flag, a Chromium API addition and a
POSIX ioctl fix: none of them is a backport. 3.23 is a stable branch, so it
will not take 8.1.2 — 3.24 already has it, which is the version the lean image
compiles.

CVE-2026-8461 is the one that matters: an out-of-bounds write in the MagicYUV
decoder, scored 8.8, described as denial of service and in some cases remote
code execution. MagicYUV travels inside Matroska and AVI, both of which are in
`VIDEO_EXTENSIONS`, and a video thumbnail is generated by browsing the folder
the file is in. A share created with uploads enabled is enough for someone with
no account to put that file there.

Neither way out is free:

- **Move the base to `node:24.21-alpine3.24`.** Every package the image
  installs exists in 3.24 and ffmpeg becomes 8.1.2 — checked against both
  indexes. But everything else moves with it: python3 3.12 to 3.14 and nasm
  2.16 to 3.01, which the native-module and ffmpeg builds go through, and
  mesa-va-gallium 25.2.7 to 26.1.6, which is the largest thing in the full
  image. `Dockerfile` also has two `FROM alpine:3.23` stages that would have to
  move in step: the source ffmpeg links against Alpine's shared libdav1d and
  libbz2 rather than statically, so build and runtime disagreeing about the
  branch is a soname waiting to fail.
- **Build ffmpeg from source in the full image too.** The pin is already
  current, and the build is already written and verified. But that build has no
  VA-API — the Dockerfile refuses the combination on purpose — so the full
  image would lose hardware acceleration, which is most of what distinguishes
  it from the lean one.

There is a third answer, which is to say the full image is for people who
control what is in the volume, and the lean one for anything exposed. That is a
position, but it is not the one the README takes today.
...
- **Nothing rebuilds the published images on a schedule.** apk resolves against
  ...
  works here.
- **`actions/setup-node` is on v7 upstream**, so the bump away from v4 noted
  below does not reach the current major either.
