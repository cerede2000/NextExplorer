...
const seedVolume = async (volume) => {
  ...
  // Renamed, copied, moved and deleted further down. Written here because the
  // API makes folders and not empty files.
  await fs.writeFile(path.join(volume, 'Documents', 'nouveau.txt'), 'du texte');
  // The document the editors are asked about. An empty Word file: enough of a
  // .docx for the routes to resolve and answer, which is all the walk needs.
  const document = new AdmZip();
  document.addFile(
    '[Content_Types].xml',
    Buffer.from(
  ...
  await fs.writeFile(path.join(volume, 'Documents', 'Rapport.docx'), document.toBuffer());
