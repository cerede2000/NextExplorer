 ...
 * included the paths an access rule hides from whoever asked, and the names a
 * listing leaves out — `_users` among them, where personal folders are kept. A
 * download is a read: it may take exactly what a listing of that folder shows,
 * whether it arrives as a zip in the browser, through a share link, or as an
 * archive written next to the folder.
 */
...
describe('a folder downloaded as a zip', () => {
  it('holds what the folder shows, and not the personal folders', async () => {
    await seed();
  ...
  it('leaves them out of each folder of a selection too', async () => {
    ...
    await fs.mkdir(path.join(volume, 'Projects', 'more', '_users', 'bob'), { recursive: true });
    await fs.writeFile(path.join(volume, 'Projects', 'more', '_users', 'bob', 'x'), 'private');
    await fs.writeFile(path.join(volume, 'Projects', 'more', '.DS_Store'), 'finder');
    ...
    expect(entries.filter((name) => /_users|\.DS_Store/.test(name))).toEqual([]);
  });
});

describe('a shared folder downloaded through its link', () => {
  it('holds what the share shows, and not a hidden path', async () => {
    await seed();
    await hideSecret();
...
describe('an archive written next to the folder', () => {
  it('holds what the folder shows, and not the personal folders or a hidden path', async () => {
    const volume = await seed();
    await hideSecret();
