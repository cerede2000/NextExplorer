...
describe('Archive extraction', () => {
   ...
   *
   * The failure comes once the folder is in place: reading it back for the
   * answer is the step after it is placed.
   */
  const failAfterPlacingFolder = (workDir, intrude) => {
    const placedFolder = path.join(workDir, 'photos');
    const stat = fs.stat.bind(fs);
    vi.spyOn(fs, 'stat').mockImplementation(async (target, ...rest) => {
      if (path.resolve(String(target)) === placedFolder) {
        intrude?.(placedFolder);
        throw Object.assign(new Error('The volume went read-only.'), { code: 'EROFS' });
      }
      return stat(target, ...rest);
    });
  };
  ...
  it('undoes a failed extraction into the current folder without removing what someone saved in it', async () => {
    ...
    failAfterPlacingFolder(workDir, (placedFolder) =>
  ...
  it('undoes a failed extraction into the current folder entirely when nobody added anything', async () => {
    ...
    failAfterPlacingFolder(workDir);
