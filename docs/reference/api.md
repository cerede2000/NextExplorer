...
Every operation — its parameters, what it takes and what it answers, and who
may call it — is in the [API explorer](/reference/api-explorer), read from the
API's OpenAPI description. That description is also
[a file](/openapi.json) a generated client or an API tool can take, and a
running instance serves its own, for the release it runs, at
`/api/openapi.json`, to anybody, as it does `/api/features`.

It is held to the application by tests rather than kept up by hand: a route
added without being described, or described and not mounted, fails the build.
