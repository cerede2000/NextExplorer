...
- **Open a share link as a guest:** Visitors open URLs like `https://files.example.com/share/aBc123XyZ`. The Share access page shows basic information (label, type, expiration) and either auto-opens the shared item, prompts for a password (of anyone but the share's owner, whether signed in or not), or redirects to the login page for user-specific shares. Guest sessions are limited so they can only browse within the shared item.
